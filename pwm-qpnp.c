...
//add for IR
/*add below line  to get the PWM_OUT on DTEST1. force write PWM register control PWM output due to spmi or register delay affect PWM time sequence*/ 
...
/*PMIC have some Secure Registers. Those registers have a property of PMIC_LOCKED, so we can't write them direclty. 
	IF we want to control those register with PMIC_LOCKED, firstly we should wirte 0x5a to 0xD0 offset to Unlock the Secure Registers. */
	reg = 0xA5;
	...
	for (; pkt->next < pkt->length; pkt->next++)
	{
		...
		if (pkt->next & 0x01)
		{
			...
			/*Write sync register to refreshing PWM output enable register value*/
			reg = 0x1;
			rc = spmi_ext_register_writel(pwm_chip->spmi_dev->ctrl, pwm_chip->spmi_dev->sid, 
