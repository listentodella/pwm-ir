...
/*
	for (; pkt->next < pkt->length; pkt->next++) {
		...
		{
		//	pwm_disable(pkt->pwm);
		//	do_gettimeofday(&timep2);
		//	printk(" wangfajie : disable start               %ld \n", timep2.tv_sec*1000000 + timep2.tv_usec);
			qpnp_ir_pwm_config_enable();
		}
		else {
		//	pwm_enable(pkt->pwm);
		//	do_gettimeofday(&timep);
		//	printk(" wangfajie : enable start                %ld \n", timep.tv_sec*1000000 + timep.tv_usec);
			qpnp_ir_pwm_config_disable();
		}
...
//	pwm_disable(pkt->pwm);
//	qpnp_ir_pwm_config_disable();
//	printk(" wangfajie : entry ir pwm tx work\n");
//	qpnp_ir_pwm_config();
...
static int pwm_ir_tx_transmit(struct rc_dev *rdev, unsigned *txbuf, unsigned n)
{
	...
	for (i = 0; i < n; i++)
	  ...
	  //printk("pwm_ir_tx_transmit txbuf[%d] = %d \n",i,txbuf[i]);
...
//	regulator_force_disable(dev->reg);
...
#if IR_DEBUG_FS
//usage:
//sf:38000   set frenquency
//sd:50   set duty
//tx:100:200:300:400:500:...   number of time must be odd  (us)
//dispwm:0   disable pwm output
//enpwm:1   enable pwm output
static ssize_t lct_ir_debug_proc_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
	...
	if(g_ir_dev == NULL)
	{
	 ...
	 if(strcmp(operation,"sf") == 0) //set frenquency
	 {
	 ...
	 else if(strcmp(operation,"sd") == 0) // set duty
	 {
	 ...
	 else if(strcmp(operation,"dispwm") == 0) //disable pwm
	 {
	 	printk("lct_ir_debug_proc_write stop PWM output \n");
	 	pwm_disable(g_ir_dev->pwm);
	 }
	else if(strcmp(operation,"enpwm") == 0)  //enable pwm
	{
	 	printk("lct_ir_debug_proc_write stop PWM output \n");
	...
	else if(strcmp(operation,"tx") == 0)
	{
		...
		#if 0
		printk("lct_ir_debug_proc_write time count = %d \n",time_count);
		for(rc = 0;rc < time_count;rc++ )
		{
			printk("lct_ir_debug_proc_write time[%d]= %d \n",rc,time[rc]);
		}
		#endif
...
//	int test_gpio = 0;
	...
	//			test_gpio = of_get_named_gpio(pdev->dev.of_node,"qcom,test-gpios",0);
	//			printk("test gpio id = %d\n",test_gpio);
